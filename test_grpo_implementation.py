...
def test_policy_robot_head():
    ...
    print(
        f"   • Robot policies shape: {next(iter(outputs['robot_policies'].values())).shape}")
    print(f"   • State value shape: {outputs['state_value'].shape}")
    print(
        f"   • Top-N distribution shape: {outputs['top_n_distribution'].shape}")
    print(
        f"   • Complexity distribution shape: {outputs['complexity_distribution'].shape}")

    # Test robot sampling
    selected_robots = robot_head.sample_robot_selection(
        outputs, temperature=0.8)
...
def test_grpo_reward_functions():
    ...
    try:
        # Compute rewards
        rewards = reward_functions.compute_all_rewards(
            prompts, completions, ground_truth)
...
def test_grpo_integration():
    ...
    try:
        ...
        integration = GRPORobotReasoningIntegration(
            model, robot_data_dir, config)

        print(f"   • GRPO integration created successfully")
        print(
            f"   • Robot head type: {type(integration.robot_selection_head).__name__}")
...
def test_configuration_loading():
    ...
    try:
        ...
        print(
            f"   • Robot types: {list(config['robot_reasoning']['robot_types'].keys())}")
        ...
        if abs(total_weight - 1.0) < 0.01:
            ...
            print(
                f"   ⚠️ Reward weights should sum to 1.0, got {total_weight:.3f}")

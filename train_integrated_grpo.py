...
class IntegratedGRPODataset(Dataset):
    ...
    def _combine_datasets(self) -> List[Dict]:
        ...
        robot_samples = int(len(self.robot_data) * robot_weight / (robot_weight + multimodal_weight))
        ...
        if self.multimodal_data:
            multimodal_samples = int(len(self.multimodal_data) * multimodal_weight / (robot_weight + multimodal_weight))
...
class BitGenGRPOTrainer:
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            try:
                ...
                if any(dt == 'robot_reasoning' for dt in batch['data_types']):
                    robot_indices = [i for i, dt in enumerate(batch['data_types']) if dt == 'robot_reasoning']
                    
                    if robot_indices:
                        robot_prompts = [batch['prompts'][i] for i in robot_indices]
                        robot_responses = [batch['responses'][i] for i in robot_indices]
    ...
    def _evaluate_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(progress_bar):
                try:
                    # Evaluation step
                    outputs = self.grpo_trainer.evaluation_step(self.model, batch)
    ...
    def _log_metrics(self, epoch: int, metrics: Dict[str, float]):
        ...
        logger.info(f"   • Policy Loss: {metrics.get('train_policy_loss', 0):.4f}")
        logger.info(f"   • Value Loss: {metrics.get('train_value_loss', 0):.4f}")
        logger.info(f"   • Average Reward: {metrics.get('train_avg_reward', 0):.4f}")
        
        if 'eval_loss' in metrics:
            logger.info(f"   • Eval Loss: {metrics['eval_loss']:.4f}")
            logger.info(f"   • Eval Average Reward: {metrics['eval_avg_reward']:.4f}")
    ...
    def _final_evaluation(self):
        ...
        for i, scenario in enumerate(test_scenarios):
            ...
            try:
                ...
                logger.info(f"   • Selected Robots: {', '.join(result['selected_robots'])}")
                logger.info(f"   • Estimated Complexity: {result['estimated_complexity']}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="BitGen GRPO Robot Reasoning Training")
    parser.add_argument(
        "--config", 
        type=str, 
        required=True,

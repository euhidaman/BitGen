...
class PolicyOptimizedRobotHead(nn.Module):
    ...
    def __init__(self, input_dim: int, num_robots: int = 5, dropout: float = 0.1):
        ...
        # Robot types
        self.robot_types = ['Drone', 'Underwater_Robot',
                            'Humanoid', 'Robot_with_Wheels', 'Robot_with_Legs']
        ...
        self.robot_policy_heads = nn.ModuleDict({
            robot: nn.Sequential(
                ...
                nn.GELU(),
                # [not_selected, selected] probabilities
    ...
    def forward(self, fused_features: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        for robot_name, policy_head in self.robot_policy_heads.items():
            ...
            robot_selections[robot_name] = policy_dist[:, 1]
    ...
    def sample_robot_selection(self, outputs: Dict[str, torch.Tensor], temperature: float = 1.0) -> Dict[str, bool]:
        ...
        for robot_name, policy_dist in robot_policies.items():
            ...
            selected_robots[robot_name] = samples[:,
                                                  0].bool()  # True if selected
    ...
    def get_top_n_robots(self, outputs: Dict[str, torch.Tensor], n: Optional[int] = None) -> List[str]:
        ...
        all_probs = torch.stack([robot_selections[robot]
                                for robot in self.robot_types])
...
class GRPORobotReasoningIntegration(RobotReasoningIntegration):
    ...
    def __init__(self, model, robot_data_dir: str, config: Dict = None):
        ...
        hidden_dim = getattr(model, 'config', {}).get(
        ...
        self.grpo_reward_functions = GRPORobotRewardFunctions(
            self.robot_processor, config)

        logger.info("✅ GRPO Robot Reasoning Integration initialized")
        logger.info(
            f"   • Policy-based robot head: {sum(p.numel() for p in self.robot_selection_head.parameters()):,} parameters")
    ...
    def generate_robot_reasoning_with_policy(
        ...
        with torch.no_grad():
            ...
            # Get top-N robots
            top_n_robots = self.robot_selection_head.get_top_n_robots(
                outputs, n=top_n)

            # Estimate task complexity
            complexity_probs = outputs['complexity_distribution'][0].cpu(
            ).numpy()
            complexity_levels = ['simple', 'moderate', 'complex']
            estimated_complexity = complexity_levels[np.argmax(
                complexity_probs)]
    ...
    def _generate_policy_based_reasoning(
        ...
        reasoning_parts.append(
            f"Estimated Task Complexity: {complexity.title()}")

        if complexity == "complex":
            reasoning_parts.append(
                "This complex task requires careful coordination between multiple specialized robots.")
        elif complexity == "moderate":
            reasoning_parts.append(
                "This moderately complex task can be handled by one or two suitable robots.")
        else:
            reasoning_parts.append(
                "This is a straightforward task that can be efficiently completed by a single robot.")
        ...
        if any(word in task_lower for word in ['underwater', 'marine', 'seabed', 'ocean']):
            reasoning_parts.append(
                "Environment Analysis: Underwater/aquatic environment requiring specialized waterproof systems and marine navigation capabilities.")
        elif any(word in task_lower for word in ['aerial', 'sky', 'above', 'high-rise', 'roof']):
            reasoning_parts.append(
                "Environment Analysis: Aerial/elevated environment requiring flight capabilities and elevated access systems.")
        elif any(word in task_lower for word in ['rough', 'mountain', 'rocky', 'uneven', 'terrain']):
            reasoning_parts.append(
                "Environment Analysis: Challenging terrain requiring advanced stability and navigation systems.")
        elif any(word in task_lower for word in ['indoor', 'building', 'facility', 'hospital', 'office']):
            reasoning_parts.append(
                "Environment Analysis: Indoor/structured environment requiring precise navigation and human interaction capabilities.")
        else:
            reasoning_parts.append(
                "Environment Analysis: Standard operational environment with typical mobility requirements.")
        ...
        for robot in selected_robots:
            robot_key = robot if robot in robot_justifications else robot.replace(
                ' ', '_')
            if robot_key in robot_justifications:
                reasoning_parts.append(
                    f"• {robot.replace('_', ' ')}: {robot_justifications[robot_key]}")

        # Coordination strategy for multiple robots
        if len(selected_robots) > 1:
            reasoning_parts.append("Coordination Strategy:")
            reasoning_parts.append(
                "Multiple robots selected to provide complementary capabilities and ensure comprehensive task completion.")
            reasoning_parts.append(
                "Coordinated operation will maximize efficiency while providing redundancy for critical operations.")
        else:
            reasoning_parts.append("Single Robot Strategy:")
            reasoning_parts.append(
                "Task requirements can be efficiently met by this specialized robot, optimizing resource utilization.")

        # Final conclusion
        robot_names = [r.replace('_', ' ') for r in selected_robots]
        reasoning_parts.append(
            f"Conclusion: {', '.join(robot_names)} selected based on optimal capability matching and task complexity analysis.")

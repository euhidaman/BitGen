...
class GPM(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer('memory_quality', torch.ones(
            memory_size))  # Quality scores for memories
        self.register_buffer('memory_age', torch.zeros(
            memory_size))  # Age of memories (for decay)
    ...
    def _get_prior_params(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self._identity:
            prior_cov = torch.eye(
                self._code_size, device=self.memory_mean.device)
        else:
            var = torch.exp(self.memory_logvar)
            prior_cov = var * \
                torch.eye(self._code_size, device=self.memory_mean.device)
    ...
    def _sample_M(self, memory_state: Dict) -> torch.Tensor:
        ...
        try:
            ...
            eps = torch.randn(self._memory_size, self._code_size,
    ...
    def read(self, query: torch.Tensor, top_k: int = 5) -> Tuple[torch.Tensor, Dict]:
        ...
        quality_weighted_similarity = similarity * \
            self.memory_quality.unsqueeze(0)

        # Get top-k memories
        top_k = min(top_k, self._memory_size)
        top_k_scores, top_k_indices = torch.topk(
            quality_weighted_similarity, k=top_k, dim=-1)
        ...
        retrieved = torch.sum(weights.unsqueeze(-1) *
                              top_k_memories, dim=1)  # [B*L, C]
    ...
    def write(self, value: torch.Tensor, importance: Optional[torch.Tensor] = None) -> Dict:
        ...
        top_importance, write_indices = torch.topk(
            importance_flat, k=num_writes)
    ...
    def load_memory(self, path: str):
        ...
        self.memory_mean.data = memory_state['memory_mean'].to(
            self.memory_mean.device)
        self.memory_logvar.data = memory_state['memory_logvar'].to(
            self.memory_logvar.device)
        self.memory_read_count.data = memory_state['memory_read_count'].to(
            self.memory_read_count.device)
        self.memory_write_count.data = memory_state['memory_write_count'].to(
            self.memory_write_count.device)
        self.memory_quality.data = memory_state['memory_quality'].to(
            self.memory_quality.device)
        self.memory_age.data = memory_state['memory_age'].to(
            self.memory_age.device)

        print(f"Memory state loaded from {path}")
        print(
            f"Memory size: {self._memory_size}, Code size: {self._code_size}")
        print(
            f"Total reads: {self.memory_read_count.sum().item()}, Total writes: {self.memory_write_count.sum().item()}")
    ...
    def update_memory_quality(self, indices: torch.Tensor, success_rate: float):
        ...
        with torch.no_grad():
            ...
            for idx in indices.view(-1).unique():
                ...
                self.memory_quality[idx] = (
                    1 - alpha) * old_quality + alpha * success_rate

    ...
    if metadata_files:
        ...
        for metadata_file in metadata_files:
            ...
            try:
                ...
                logger.info(
                    f"      Samples: {metadata.get('num_samples', 'unknown')}")
                logger.info(
                    f"      Dummy vision: {metadata.get('use_dummy_vision', 'unknown')}")
                logger.info(
                    f"      Real features: {metadata.get('extract_vision_features', 'unknown')}")
            except Exception as e:
...
def clear_cache(config: Dict, confirm: bool = False):
    ...
    if not confirm:
        response = input(
            "⚠️  This will delete all cached vision features. Continue? [y/N]: ")
...
def rebuild_cache(config: Dict, use_real_vision: bool = False):
    ...
    if use_real_vision:
        logger.info(
            "🎨 Rebuilding cache with REAL vision features (slower but higher quality)")
...
def main():
    parser = argparse.ArgumentParser(
        description="Manage vision features cache")
    ...
    clear_parser = subparsers.add_parser(
        'clear', help='Clear vision features cache')
    clear_parser.add_argument(
        '--yes', action='store_true', help='Skip confirmation prompt')

    # Rebuild command
    rebuild_parser = subparsers.add_parser(
        'rebuild', help='Rebuild vision features cache')
